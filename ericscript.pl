...
#    $outputfolder = abs_path($outputfolder);
	...
	my $abs_outputfolder = abs_path($outputfolder);
	my $simcommand="R --slave --args $readlength,$abs_outputfolder,$ericscriptfolder,$verbose,$insize,$sd_insize,$ngenefusion,$min_cov,$max_cov,$nsims,$be,$ie,$background_1,$background_2,$nreads_background,$dbfolder,$refid,$fusionlist < $ericscriptfolder/lib/R/SimulateFusions.R";
	system($simcommand);	
