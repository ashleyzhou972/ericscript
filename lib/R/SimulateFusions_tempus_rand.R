...
#Ashley 20210121
all_transcripts <- all_transcripts[all_transcripts$tx1 %in% TranscriptNames & all_transcripts$tx2 %in% TranscriptNames]
...
for (jj in 1: nsims) {
	...
	mycount <- 0
	while (flag == 1) {
		sampled_index <- sample(1:nrow(all_transcripts), ngenefusion)
		trans1 <- all_transcripts%tx1[sampled_index]
		...
		print(gene1)
		trans2 <- all_transcripts%tx2[sampled_index]
		for (ii in 1: ngenefusion) {
