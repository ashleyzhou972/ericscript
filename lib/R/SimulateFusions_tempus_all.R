...
all_transcripts <- readRDS("/home/ec2-user/data/bio-5943-mojo-synthetic/all_transcript_pair_combn.rds")
...
for (jj in 1: nsims) {
	...
	while (flag == 1) {
		...
		#if (nrow(sel_transcripts) != ngenefusion) {stop("Number of total fusions doesn't match input transcripts\n")}
		#trans1 <- sample(TranscriptNames, ngenefusion)
		#trans1 <- sel_transcripts$tx1[start:(start + ngenefusion - 1)]
		#trans1 <- sel_transcripts$tx1
		trans1 <- c("ENST00000398585","ENST00000396373")
		...
		#trans2 <- sel_transcripts$tx2[start:(start + ngenefusion - 1)]
		#trans2 <- sel_transcripts$tx2
		trans2 <-c("ENST00000417133","ENST00000524377")
		for (ii in 1: ngenefusion) {
